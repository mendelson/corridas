...
9. **`horario`** — **not required** (policy change 2026-07-11): events without a published start time are **accepted**, regardless of how close the event date is. Scrapers must still search and extract the horário with **maximum effort** (structured fields, deep-fetching detail/inscription pages) — the field is only omitted when the information is genuinely impossible to obtain. The pipeline keeps such events and re-checks every run, filling the horário in as soon as any source publishes it. `scraper.utils.horario_required()` now always returns `False`; the previous strict rule (horário mandatory within a 3-month window, enforced by `main.py`'s `_drop_events_without_horario`, `test_source.py` and `test_site.py`) is preserved commented-out in that function and can be restored by re-enabling it — the call sites were kept intact.

**All fields above (except `horario`, see item 9) are mandatory with zero tolerance — there are no percentage/ratio thresholds anywhere.** A single non-compliant event fails the test.
...
- Missing `horario` is **not a failure** (policy change 2026-07-11 — `horario_required()` always returns `False`); scrapers must still extract it with maximum effort, but events without one are accepted.
