...
def _search_largada_esportiva(today: str) -> list[Corrida]:
    ...
    for ev in events:
        ...
        pais="BR",
            ...
            inscricoes_abertas=None,
            periodo_inscricao=None,
            fontes=[FonteInfo(nome="Largada Esportiva", link_evento=link, links_inscricao=[link], tipo="organizador")],
            miss_count=0,
            first_seen_at=now,
            updated_at=now,
        ))
...
def _parse_ts_event(ev: dict, today: str) -> Corrida | None:
    ...
    return Corrida(
        ...
        inscricoes_abertas=None,
        periodo_inscricao=None,
        fontes=[FonteInfo(nome=SOURCE_NAME, link_evento=link, links_inscricao=[link], tipo="organizador")],
        miss_count=0,
        first_seen_at=now,
        updated_at=now,
    )
