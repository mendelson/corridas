"""Scraper for minhasinscricoes.com.br"""
...
def scrape() -> list[Corrida]:
    ...
    # Deduplicate by id (nav links like "calendario-completo" appear multiple times)
    seen: set[str] = set()
...
def _find_events(soup):
    for sel in [".event-card", ".race", ".card", "article", ".item", ".evento"]:
        els = soup.select(sel)
        if els:
            return els
    return []
...
def _parse_event(el) -> Corrida | None:
    ...
    cidade = localizacao.split(",")[0].strip() if "," in localizacao else localizacao.split("-")[0].strip()
